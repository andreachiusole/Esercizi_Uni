...
// L'utilizzo di queste strutture è uguale alle altre strutture
// unica differenza è che si sceglie la dimensione rispetto al numero di bit per ogni campo.
// Questo permette di avere una maggiore efficienza della memoria.

struct info{
    // Questo numero intero è senza segno ed occupa 4 bit
    unsigned f:4;
    //I tipi ammessi sono quelli per gli interi
    // double g:2; Questo non è ammesso perché interpretato in maniera diversa dagli int
...
// Anche per questa struttura si accede ai campi come per le altre strutture con il '.'
